...
//выгружает контейнер без крышки
//перед этой командой должна выполняться команда CHANGING_BATTERY
//контролируемое перемещение с указанием cmx, cmy, cmz
//т.е. выгрузка происходит от туда куда приехала каретка
//далее производит СНЯТИЕ крышки с зарядки.
//возвращает контейнер С КРЫШКОЙ в заданную ячейку
//т.е. по внешним параметрам команды
 static void cmOcls() 
    {  vTaskDelay(200);
       if ((!ZUN)&&(!XUN)&&(!YUN)&&(cmx<1)&&(cmy<4)&&(cmz<4)&&(cmkr<1)&&((stY=="3")||(stY=="1")))
...
//выгружает контейнер с крышкой.
//перед этой командой должна выполняться команда CHANGING_BATTERY
//контролируемое перемещение с указанием cmx, cmy, cmz
//т.е. выгрузка происходит от туда куда приехала каретка
//производит УСТАНОВКУ крышки на зарядку.
