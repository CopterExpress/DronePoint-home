...
while True:
   try:  #UNLOAD_TO_USER CUSTOM_MODE операция меняет стэйт
         #vynet iz iacheyki i vygruzit polzavatelu
         #в новом дронпоинте X всегда 0 и Kr0 всегда 0
         print("UNLOAD_TO_USER X0Y3Z2")
         master.mav.command_long_send(master.target_system, master.target_component,
                                  mavutil.mavlink.MAV_CMD_DO_SET_MODE, 1,
                                  mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED,
                                  CUSTOM_MODE_UNLOADING_TO_USER, #8 стэйт
                                  0, 3, 2, 0, 0) 
         time.sleep(5)
         while CCSM!=12:
           pass
         time.sleep(7)
         #GET_FROM_USER CUSTOM_MODE операция меняет стэйт
         #primet u polzovatelai i vstavit v iacheyku
         print("GET_FROM_USER X0Y3Z2")
         master.mav.command_long_send(master.target_system, master.target_component,
                                  mavutil.mavlink.MAV_CMD_DO_SET_MODE, 1,
                                  mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED,
                                  CUSTOM_MODE_GETTING_FROM_USER, #7 стэйт
                                  0, 3, 2, 0, 0) 
         time.sleep(5)
         while CCSM!=12:
           pass
